...
```{r}
requete3 <- mdb$aggregate('[
    
    {"$unwind":"$visit_motives"},
    {"$unwind":"$visit_motives.slots"},
    "$match":{"visit_motives.slots":{"$gte":ISODate("2022-01-26"),
                                     "$lt":ISODate("2022-01-30")}}
]')
```
...
leaflet(data = result) %>% setView(lng = Rennes[1], lat = Rennes[2], zoom = 8) %>% 
  addTiles() %>%
